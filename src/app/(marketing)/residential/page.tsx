...
import { residentialHubQuery } from "@/lib/sanity/queries";
import type { ResidentialHub, RichImage } from "@/lib/sanity/types";
import { createMetadata } from "@/lib/seo/metadata";

const loadResidentialHub = cache(async () =>
  sanityFetch<ResidentialHub | null>({
    query: residentialHubQuery,
    tags: ["residential-hub"],
  }),
);
...
export async function generateMetadata(): Promise<Metadata> {
  const data = await loadResidentialHub();
  const seo = data?.seo;
  const title = seo?.metaTitle?.trim() || "Residential solar | Veylon Energy";
...
export default async function ResidentialPage() {
  ...
  return (
    <>
      ...
      <ValueProps items={data.valueProps} segment="residential" />
      <HowItWorks steps={data.howItWorks} />
      <CalculatorEmbed data={data.calculatorEmbed} segment="residential" />
      <ComponentPartners partners={data.componentPartners} />
      <HubFeaturedProjects projects={data.featuredProjects} segment="residential" />
      <HubFaqs faqs={data.faqs} segment="residential" />
